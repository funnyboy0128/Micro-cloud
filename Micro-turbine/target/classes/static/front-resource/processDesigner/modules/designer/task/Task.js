...
draw2d.Task.prototype.figureDoubleClick = function() {
	...
	openProperties(event.getId(), 'taskProperties');
};
...
draw2d.Task.prototype.onDragstart = function(x, y) {
	...
	/*扩展拖拽if (this.textarea !== null) {
		...
		if (this.originalHeight == -1) {
			if (this.canDrag === true && x < parseInt(this.textarea.style.width, 10)
					&& y < (parseInt(this.textarea.style.height, 10) + 28)) {
				return true;
			}
		} else {
			return _5017;
...
/*
 * draw2d.Task.prototype.onDoubleClick=function(){ var tid = this.getId();
...
draw2d.Task.prototype.getContextMenu = function() {
	...
	var menu = new draw2d.ContextMenu(100, menuHeight);
	var data = {
		task : this
	};
	menu.appendMenuItem(new draw2d.ContextMenuItem("属性", "properties-icon",
			data, function(x, y) {
				var data = this.getData();
				var task = data.task;
				if (typeof openProperties != "undefined") {
					openProperties(task.getId(), 'taskProperties');
				}
			}));
    if (m_processIsSave) {
		menu.appendMenuItem(new draw2d.ContextMenuItem("附件设置", "properties-icon",
	        data, function(x, y) {
	            var data = this.getData();
	            var task = data.task;
	            if (typeof openProperties != "undefined") {
	                openDocConfig(task.getId(), task.taskName);
	            }
	        }));
    }
	menu.appendMenuItem(new draw2d.ContextMenuItem("删除", "icon-remove", data,
			function(x, y) {
				var data = this.getData();
				var task = data.task;
				var tid = task.getId();
