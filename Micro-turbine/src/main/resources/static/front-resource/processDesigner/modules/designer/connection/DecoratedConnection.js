...
draw2d.DecoratedConnection.prototype.getConditionXML = function() {
	...
	if (this.condition != null && this.condition != '') {
		...
		var condition_value = this.condition.replace(/(^\s*)|(\s*$)/g, "");
		xml = '<conditionExpression xsi:type="tFormalExpression"><![CDATA['
				+ condition_value + ']]></conditionExpression>\n';
	}
	return xml;
}
draw2d.DecoratedConnection.prototype.toXML = function() {
	...
	if (lineName != null && lineName != "")
		name = lineName;
	var xml = '<sequenceFlow id="' + this.lineId + '" name="' + name
...
draw2d.DecoratedConnection.prototype.toBpmnDI = function() {
	var xml = '<bpmndi:BPMNEdge bpmnElement="' + this.lineId
			+ '" id="BPMNEdge_' + this.lineId + '">\n';
	...
	for (var i = 0; i < points.size; i++) {
		xml = xml + '<omgdi:waypoint x="' + points.get(i).x + '" y="'
				+ points.get(i).y + '" />\n';
	}
...
draw2d.DecoratedConnection.prototype.onDoubleClick = function() {
	openProperties(this.getId(), 'lineProperties');
};
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("属性", "properties-icon",
			data, function(x, y) {
				...
				if (typeof openProperties != "undefined") {
					openProperties(line.getId(), 'lineProperties');
				}
			}));
