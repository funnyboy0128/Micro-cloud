...
draw2d.MyCanvas.prototype.showConnectionLine=function(x1, y1, x2, y2){
	...
	if (connectionLine.canvas === null) {
		draw2d.Canvas.prototype.ddFigure.call(this, connectionLine);
	}
};
*/
...
draw2d.MyCanvas.prototype.getContextMenu=function(){
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("属性", "properties-icon",data,function(x,y)
	{
		...
		openProperties(pid,'processProperties',workflow.process);
	}));
	if (m_processIsSave) {
	    menu.appendMenuItem(new draw2d.ContextMenuItem("附件设置", "properties-icon",data,function(x,y)
	    {
	        var data = this.getData();
	        var workflow = data.workflow;
	        var pid = workflow.process.id;
