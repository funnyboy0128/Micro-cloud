 ...
 * @param {Any} variable  需要检测的变量值，必选
 */
function testPositiveInt(variable) {
    let reg = /^[1-9]+\d*$/g;
    let checkRight = testReg(reg, variable);
    if (checkRight) {
        checkRight = variable < 2147483647;
    }
    return checkRight;
}
/**
 ...
 * @param {Any} variable  需要检测的变量值，必选
 */
function testHasChinese(variable) {
    let reg = /[\u4e00-\u9fa5]{1,}/g;
    return testReg(reg, variable);
}
...
/**
 * 判断是否只为数字、26个英文字母，或者中文组成的字符串
 * @param {Any} variable  需要检测的变量值，必选
 */
function testOnlyNumLetterChinese(variable) {
    let reg = /^[\w\u4e00-\u9fa5]*$/g;
    return testReg(reg, variable);
}
/**
 ...
 * @param {Any} variable  需要检测的变量值，必选
 */
function testPort(variable) {
    let reg = /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{4}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/g;
    return testReg(reg, variable);
}
...
function test6To32NumbersAndOrLetters(variable) {
    let reg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,32}$/g;
    return testReg(reg, variable);
}