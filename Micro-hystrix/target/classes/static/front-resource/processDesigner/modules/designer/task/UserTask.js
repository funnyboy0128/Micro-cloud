...
draw2d.UserTask.prototype.getStartElementXML=function(){
	...
	if (this.isUseExpression) {
		...
		if (this.candidateUsers != null && this.candidateUsers.size > 0) {
			// 注意多个循环操作
			let userIds = "";
			for (let k=0;k<this.candidateUsers.size;k++){
				if (k === 0){
                    userIds += this.candidateUsers.get(k).sso;
				}else{
                    userIds += ',' + this.candidateUsers.get(k).sso;
				}
			}
            xml = xml + 'activiti:candidateUsers="' + userIds + '" ';
		}

		if (this.candidateGroups != null && this.candidateGroups.size > 0) {
			// 注意多个循环操作
            let roleIds = "";
            for (let k=0;k<this.candidateGroups.size;k++){
                if (k === 0){
                    roleIds += this.candidateGroups.get(k);
                }else{
                    roleIds += ',' + this.candidateGroups.get(k);
                }
            }
			xml = xml + 'activiti:candidateGroups="' + roleIds + '" ';
		}
	}
...
draw2d.UserTask.prototype.getMultiInstanceXML=function(){
    ...
    //xml+='<multiInstanceLoopCharacteristics  isSequential="false" activiti:collection="${flowUtil.stringToList(applyUserIdList)}"  activiti:elementVariable="applyUserId"  ></multiInstanceLoopCharacteristics>';
    xml+='<multiInstanceLoopCharacteristics isSequential="' + this.task_extend + '" activiti:collection="' + this.taskId.replace( /-/g, '_') + '_assignees"  activiti:elementVariable="' + this.taskId.replace( /-/g, '_') + '_assignee"  ></multiInstanceLoopCharacteristics>';
	return xml;
};
