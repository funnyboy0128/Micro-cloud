...
draw2d.LineStartResizeHandle.prototype.onDrop = function(_5b27) {
	...
	if (line instanceof draw2d.Connection) {
		...
		// 已有连线拖动后检测连线是否重复-源节点检测
		let lines = workflow.getLines();
		for (var i = 0; i < lines.getSize(); i++) {
			var tmpLine = lines.get(i);
			if (tmpLine.lineId != line.lineId) {
				var currTarget = line.getTarget().getParent();
				var tmpSource = tmpLine.getSource().getParent();
				var tmpTarget = tmpLine.getTarget().getParent();
				if (tmpSource.id == _5b27.parentNode.id && tmpTarget.id == currTarget.id) {
					// 当前连线重复
...
draw2d.LineEndResizeHandle.prototype.onDrop = function(_58fe) {
	...
	if (line instanceof draw2d.Connection) {
		...
		// 已有连线拖动后检测连线是否重复-目标节点检测
		let lines = workflow.getLines();
		for (var i = 0; i < lines.getSize(); i++) {
			var tmpLine = lines.get(i);
			if (tmpLine.lineId != line.lineId) {
				var currSource = line.getSource().getParent();
				var tmpSource = tmpLine.getSource().getParent();
				var tmpTarget = tmpLine.getTarget().getParent();
				if (tmpSource.id == currSource.id && tmpTarget.id == _58fe.parentNode.id) {
					// 当前连线重复
					fnTip("当前节点间已经存在连线，请勿重复添加!", 2000);
					return;
				}
