...
function fnWait(type) {
    ...
    const layerIndex = layer.load(type, {
        zIndex: modalIndex
    });
 ...
 *      @param01 {Number} animType 弹窗动画类型的索引值，可缺省
 *                       0	平滑放大。默认
 *                       1	从上掉落
 *                       2	从最底部往上滑入
 *                       3	从左滑入
 ...
 *                       5	渐显
 *                       6	抖动
 *      @param02 [String,Array] offset 弹窗相对当前窗口位置，可缺省
 *                       'auto'	默认坐标，即垂直水平居中
 ...
 *                       'rb'	快捷设置右下角
 */
function fnAlert(msg, modalIndex, isCloseAll, modalObj) {
    if (!msg) {
        return;
    }
    const layer = layui.layer;
    ...
    if (!modalObj.hasOwnProperty('offset')) {
        modalObj.offset = 'auto';
    }
    ...
    let layerIndex = 0;
    const res = layer.open({
        id: 'fnAlert',
        type: 1,
        title: '提示',
        anim: modalObj.animType,
        offset: modalObj.offset,
 ...
 *                       'rb'	快捷设置右下角
 */
function fnTip(msg, timeEnd, modalIndex, modalObj) {
    if (!msg) {
        return;
    }
    const layer = layui.layer;
    ...
    if (modalObj.type === undefined || !Number(modalObj.type) || (Number(modalObj.type) && modalObj.type > 4)) {
        modalObj.type = 0; // 默认type 为0
    }

    if (modalObj.icon === undefined || !Number(modalObj.icon) || (Number(modalObj.icon) && modalObj.icon > 16)) {
        modalObj.icon = -1; // 默认icon 为-1
    }

    if (modalObj.shade === undefined || (!Number(modalObj.shade) && !Array(modalObj.shade))) {
        modalObj.shade = 0; // 默认shade 为0
    }

    const layerIndex = layer.msg(msg, {
        id: 'fnTip',
        zIndex: modalIndex,
        time: timeEnd,
        anim: modalObj.animType,
        offset: modalObj.offset,
        tipsMore: modalObj.tipsMore ? modalObj.tipsMore : false,
        type: modalObj.type,
        icon: modalObj.icon,
        shade: modalObj.shade
    }, function () {
        return true;
    });
    window.layerIndex = layerIndex;
 ...
 *                       5	渐显
 *                       6	抖动
 *      @param09 {Boolean} drag 模态窗是否需要能够拖拽 ，可缺省
 *      @param10 [String,Array] offset 弹窗相对当前窗口位置，可缺省
 *                       'auto'	默认坐标，即垂直水平居中
 *                       '100px'	只定义top坐标，水平保持居中
 *                       ['100px', '50px']	同时定义top、left坐标
 *                       't'	快捷设置顶部坐标
...
function fnAlertShow(msg, modalObj, showCallback, BCallback, cancelCallback, hideCallback, dragCallback) {
    ...
    const maxHeight = $body.height() - 30;
    if (!!modalObj.height && modalObj.height > maxHeight) {
        modalObj.height = maxHeight;
    }
    const maxWidth = $body.width();
    if (!!modalObj.width && modalObj.width > maxWidth) {
        modalObj.width = maxWidth;
    }
    const tempArr = [];
    if (modalObj.needB) {
        tempArr.push('确定');
    ...
    let layerIndex = 0;
    const dragFun = function (layero) {
        let tempCheck = true;
        if (typeof dragCallback === 'function') {
            const temp = dragCallback(layero);
            if (temp !== undefined) {
                tempCheck = temp;
            }
        }
    ...
    layer.open({
        ...
        yes: function (index, layero) {
            ...
            if (typeof BCallback === 'function') {
                const temp = BCallback(index, layero, function () {
                    layer.close(layerIndex);
                });
                if (temp !== undefined) {
                    tempCheck = temp;
    ...
    // 保证可搜索下拉框为禁用状态时渲染出来的input不可编辑
    keepRenderRight('fnAlertShowFilter');
    return layerIndex;
}
 ...
 * @return {Number} layerIndex 当前弹窗的index值
 */
function fnAlertHide(msg, modalIndex, hideCallback, modalObj) {
    if (!msg) {
        return;
    }
    const layer = layui.layer;
 ...
 * @return {Number} layerIndex 当前弹窗的index值
 */
function fnConfirm(msg, BCallback, modalIndex, QCallback, hideCallback, modalObj) {
    if (!msg) {
        return;
    }
    const layer = layui.layer;
    ...
    layer.open({
        ...
        btn2: function (index, layero) {
            ...
            if (typeof QCallback === 'function') {
                const temp = QCallback(index, layero, function () {
                    layer.close(layerIndex);
                });
                if (temp !== undefined) {
                    tempCheck = temp;
 ...
 *                       6	抖动
 *      @param04 {Boolean} showMax 模态窗是否需要最大化最小化按钮 ，可缺省
 *      @param05 [String,Array] offset 弹窗相对当前窗口位置，可缺省
 *                       'auto'	默认坐标，即垂直水平居中
 *                       '100px'	只定义top坐标，水平保持居中
 *                       ['100px', '50px']	同时定义top、left坐标
 *                       't'	快捷设置顶部坐标
...
function fnFormDialog(title, obj, showCallback, BCallback, modalIndex, modalObj, cancelCallback, hideCallback, dragCallback) {
    ...
    layer.open({
        ...
        yes: function (index, layero) {
            ...
            let tempCheck = true;
            if (typeof BCallback === 'function') {
                const tempObj = translateFormToBigObject(index, layero, obj);
 ...
 *                       5	渐显
 *                       6	抖动
 *      @param07 [String,Array] offset 弹窗相对当前窗口位置，可缺省
 *                       'auto'	默认坐标，即垂直水平居中
 *                       '100px'	只定义top坐标，水平保持居中
 *                       ['100px', '50px']	同时定义top、left坐标
 *                       't'	快捷设置顶部坐标
 ...
 * @return {Number} layerIndex 当前弹窗的index值
 */
function fnIfram(src, modalObj, hideCallback, successCallback) {
    if (!src) {
        return;
    }
    const layer = layui.layer;
    ...
    layer.open({
        type: 2,
        id: 'fnIfram' + modalObj.modalIndex,
        title: modalObj.title,
        area: [modalObj.width + 'px', modalObj.height + 'px'],
        content: [src, 'no'],
        maxmin: true,
...
function layerCloseNow(layerType) {
    if (layerType !== undefined || layerType !== null) {
        ...
        const tempIndex = window.layerIndex;
 ...
 *  isSilence {Boolean} 是否给文本框自动添加最大长度属性限制，可缺省
...
function getTextInput(item) {
    ...
    const pfor = item.pfor;
    const maxLength = item.maxLength;
    const isSilence = item.isSilence === undefined ? true : item.isSilence;
    const dataType = item.dataType;
    const inputType = dataType === 'number' ? dataType : 'text';
    ...
    // if(inputType ==='number'){
    //   res += "onkeypress='return( /^[-\+]?\d+$/.test(String.fromCharCode(event.keyCode) ) )'"
    // }
    if (pfor) {
        res += ' pfor="' + pfor + '"';
    }
    if (!!maxLength && !!isSilence) {
...
function getSelectInput(item, type) {
    ...
    const pfor = item.pfor;
    const selectEditable = item.editable;
    const selectMultiple = item.multiple;
    const selectSearch = item.search;
    ...
    if (!!data && !hasChecked && selectEditable) {
        res += '  <option index="' + option.length + '" value="' + data + '" selected="true">' + data + '</option>';
    }
...
function getCheckboxInput(item) {
    ...
    items.forEach(function (option, index) {
        const tempPfor = option.pfor;
        const tempVisible = option.visible;
        const tempValue = option.value || option.title;
...
function translateFormToObject(modalIndex, currentModal, type, selector) {
    ...
    const currentObj = $(currentModal).length > 0 ? $(currentModal) : $('#layui-layer' + modalIndex);
    let allInput = {};
    if (type === undefined) {
        type = 0;
    }
    ...
    if (allInput.length > 0) {
        for (let i = 0; i < allInput.length; i++) {
            let _name = '';
            let _value = '';
            const nowItem = $(allInput[i]).find('.form-control');
            if (nowItem.length === 0) {
                // 对radiobox的判断和取值
                const obj = $(allInput[i]).find('.layui-form-radioed');
                if (obj.length > 0) {
...
function translateFormToBigObject(modalIndex, currentModal, obj) {
    ...
    const currentObj = $(currentModal).length > 0 ? $(currentModal) : $('#layui-layer' + modalIndex);
    const allInput = currentObj.find('.layui-layer-content>.layui-form-pane').children('.layui-form-item');
    if (allInput.length > 0) {
        for (let i = 0; i < allInput.length; i++) {
            ...
            if (obj[i].type !== 'select') {
                ...
                if (nowItem.hasClass('drpEditable')) {
                    ...
                    const checkDom = $('select[lay-filter="' + obj[i].id + '"]').siblings('.layui-form-select');
                    if (checkDom.length > 0) {
                        nowItem = checkDom.find('.layui-this').text();
                        obj[i].data = nowItem.trimAll();
                    } else {
                        const checkCurrData = nowItem.select2('data');
...
function checkInputCheckType(data, dataType) {
    ...
    switch (dataType) {
        ...
        case 'int':
        {
            if (typeof testPositiveInt !== undefined) {
